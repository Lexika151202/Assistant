...
- **generate-test-cases**: `references\generate-test-cases.md`
- **update-test-cases**: `references\update-test-cases.md`
...
- If the user selects **generate-test-cases**, load `references/generate-test-cases.md`. 
- If the user selects **update-test-cases**, check if the `testcases` directory exists or provided by user. If it exists, load `references/update-test-cases.md`. If it does not exist, ask the user to provide the `testcases` directory.

## Input Contract
For **generate-test-cases** workflow:
- The `qc-uc-review` readiness report `audited-v[N].md` at `docs\usecases\<module>\<UC-ID>`  - read the latest version
- (Optional) Scenarios generated by `qc-scenario-design` at `docs\qc\scenarios\<module>\<UC-ID>` - read the latest version
- The xlxs template file: `qc-tc-design\templates\Testcase_template.xlsx`

For **update-test-cases** workflow:
- The `testcases` directory - at `docs\qc\testcases\<module>\<UC-ID>`
- The `qc-uc-review` readiness report `audited-v[N].md` at `docs\usecases\<module>\<UC-ID>`  - read the latest version
- (Optional) Scenarios generated by `qc-scenario-design` at `docs\qc\scenarios\<module>\<UC-ID>` - read the latest version
- The xlxs template file: `qc-tc-design\templates\Testcase_template.xlsx`

## Output Contract
All output files should be saved in the `docs\qc\testcases\<module>\<UC-ID>` directory.
For **generate-test-cases** workflow:
| Artifact        | Output Path                                                                 |
| --------------- | --------------------------------------------------------------------------- |
| Test Cases md   | `[UC-ID]_[feature-name]_testcases_v[N].md`                                 |
| Test Cases xlxs | `[UC-ID]_[feature-name]_testcases_[YYYYMMDD]_v[N].xlsx`                     |
| Summary         | `[UC-ID]_[feature-name]_testcases_summary_[YYYYMMDD]_v[N].md`               |

For **update-test-cases** workflow:
| Artifact           | Output Path                                                                 |
| ---------------    | --------------------------------------------------------------------------- |
| Test Cases md updated | `[UC-ID]_[feature-name]_testcases_[YYYYMMDD]_v[N].md`                     |
| Test Cases xlxs updated | `[UC-ID]_[feature-name]_testcases_[YYYYMMDD]_v[N].xlsx`                     |
| Summary            | `[UC-ID]_[feature-name]_testcases_summary_[YYYYMMDD]_v[N].md`               |
