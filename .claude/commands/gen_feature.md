---
description: "Phase 2 — Gen code React Web cho 1 feature từ Figma. Tạo pages bám sát 100% thiết kế. Ví dụ: /gen_feature 1.1 hoặc /gen_feature Đăng nhập"
---
...
3. **Đọc `spec_files`** (nếu có) → load ảnh tham chiếu để AI nhìn trực tiếp
...
### 2. Gen pages & components

- Tạo file React (.tsx) cho từng function trong `app/src/pages/`
...
- Sử dụng Tailwind CSS classes + theme tokens
- Gọi skill **`gen_screen`** với design context đã xây dựng

### 3. Xử lý multiple views per function

Nếu function có nhiều `figma_nodes` (ví dụ: List view + Form view):
- List view → page chính
- Form view → modal/dialog hoặc detail page
- Wire up navigation giữa chúng bằng React Router

### 4. Cập nhật Router
- Thêm import + Route vào `app/src/router.tsx`
- Đảm bảo route paths nhất quán
...
functions:
  - id: '76'
    status: done
    file_path: app/src/pages/HoSo/HoSoListPage.tsx
    note: List page with tab filter
  # ...
```
