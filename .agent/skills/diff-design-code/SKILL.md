...
description: "So sánh thiết kế mới (Figma MCP hoặc specs mới) với code React Web hiện tại. Trả về danh sách thay đổi cụ thể. Không sửa code."
...
- **Styles**: Tailwind classes, inline styles, theme tokens
- **Content**: Text strings
- **Logic**: State, handlers, API calls (ghi nhận nhưng KHÔNG so sánh)

### Bước 2 — So sánh với design mới

**Nhánh Figma:** So sánh reference code + metadata từ `get_design_context` với code React Web hiện tại.

**Nhánh Specs:** So sánh specs mới (phân tích ảnh/text) với code React Web hiện tại + `spec_description` cũ.
