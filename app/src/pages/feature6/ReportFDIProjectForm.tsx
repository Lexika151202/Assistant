// Feature: 6 - Danh mục dự án ĐTNN đang có NĐT quan tâm năm
// Screen: Form Lập / Chỉnh sửa / Xem báo cáo (Mẫu A.IV.4)
// Figma Node: 2207:2100 (Tab 4 active — default)
// Functions: 29 (Lập+Lưu nháp+Gửi), 31 (Chỉnh sửa), 34 (In)
...
function Tab3CamKetDauTu() {
  return (
    <Card padded={false} className="mb-6">
      <div className="p-6 pb-4">
        <CardTitle>Mẫu B.IV.4: Báo cáo tình hình thực hiện các cam kết/thỏa thuận</CardTitle>
        <p className="text-[13px] text-[#4a5565] mt-1">Hợp tác đầu tư/Chủ trương đầu tư</p>
      </div>
      <div className="px-6 pb-6 text-[14px] text-[#6b7280] italic">
        Nội dung Tab 3 — Cam kết đầu tư (xem Mẫu B.IV.4)
      </div>
    </Card>
  );
}
...
function Tab4DuAnDTNN({
  ...
  tenDonVi: string;
  nam: string;
  onTenDonViChange: (v: string) => void;
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <Label required={!isView} htmlFor="tenDonViToChuc">Tên đơn vị tổ chức/Cá nhân báo cáo</Label>
            <Input
              id="tenDonViToChuc"
              placeholder="Nhập tên cơ quan lập báo cáo"
              value={tenDonVi}
              onChange={(e) => onTenDonViChange(e.target.value)}
              readOnly={isView}
...
export default function ReportFDIProjectForm({ mode = 'create' }: ReportFDIProjectFormProps) {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const isView = mode === 'view';
  const isCreate = mode === 'create';

  // Tab 4 is the default active tab for feature 6 (Figma 2207:2100 — Tab 4 active)
  const [activeTab, setActiveTab] = useState<ActiveTab>('tab4');

  // Shared fields
  const [tenDonVi, setTenDonVi] = useState(isCreate ? '' : 'Bộ Kế hoạch và Đầu tư');
  ...
  function handleSaveDraft() { alert('Đã lưu nháp báo cáo A.IV.4.'); }
  function handleSend() {
    if (confirm('Gửi báo cáo? Sau khi gửi sẽ không thể chỉnh sửa trực tiếp.')) {
      alert('Đã gửi báo cáo A.IV.4.');
      navigate('/feature-6');
    }
  }
  ...
  return (
    <Container>
      ...
      {/* Tab bar — Figma 2207:2119 — Tab 4 active by default */}
      ...
      {/* ACTION BAR — Figma 2207:2339 (Tab 4 — Hủy / Xem / Lưu nháp / Gửi báo cáo) */}
      <div className="flex items-center justify-end gap-4 py-4 border-t border-[#e5e7eb] mt-6">
        <Button variant="outline" onClick={() => navigate('/feature-6')}>
          Hủy
        </Button>
        <Button
          variant="outline"
          onClick={() => id ? navigate(`/feature-6/${id}/view`) : alert('Lưu nháp trước để xem')}
        >
          Xem
        </Button>
        ...
        {!isView && (
          <>
            ...
            {/* Gửi báo cáo — primary red — Figma 2207:2353 */}
        ...
        {isView && (
          <Button variant="outline" onClick={() => navigate(`/feature-6/${id}/edit`)}>
            Chỉnh sửa
          </Button>
        )}
      </div>
