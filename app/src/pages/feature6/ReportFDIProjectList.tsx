...
// Functions: 30 (Xem+Lọc+Export+Import), 32 (Xem+Vòng đời), 33 (Phê duyệt/Từ chối)
...
function ProgressBadge({ value }: { value: string }) {
  ...
  const isComplete = parts.length === 2 && parts[0] === parts[1];
  return (
...
export default function ReportFDIProjectList() {
  ...
  return (
    <Container>
      <Breadcrumb items={[
        ...
        { label: 'Báo cáo dự án đầu tư nước ngoài tại Việt Nam' },
      ]} />
              ...
              {group.periodStatus !== 'Chưa tới hạn nộp báo cáo' && (
                <div className="flex items-center gap-2 ml-4">
                  <Button
                    size="sm"
                    variant="primary"
                    onClick={() => navigate('/feature-6/new')}
                  >
                    Lập báo cáo
                  </Button>
                  <button
                            ...
                            <tr key={file.id} className="border-t border-[#e5e7eb] hover:bg-[#f9fafb]">
                              ...
                              <td className="px-2 py-3">
                                <FileRowActions
                                  ...
                                  onEdit={() => navigate(`/feature-6/${file.id}/edit`)}
                                  onView={() => navigate(`/feature-6/${file.id}/view`)}
